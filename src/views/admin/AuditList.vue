...
<script>
    import {
        listDelegateRecords, delDelegate, getDelegateByTaskID, addDelegateauditrecords, updateDelegateauditrecords, getTaskCategories, withdrawReleaseByTaskIDAdmin
    } from "@/api/";
    ...
    export default {
        ...
        methods: {
            ...
            async deleteRecordAdmin() {
                ...
                console.log("删除委托", id);
                await executeConfirmedRequest(delDelegate, id, "是否确认删除该委托？", "提示", "警告", "操作警告", "操作失败，请稍后重试", "操作已取消");
                this.getList();
            ...
            async withdrawReleaseAdmin() {
                await executeConfirmedRequest(withdrawReleaseByTaskIDAdmin, this.form.taskId, "是否确认撤销发布?", "提示", "警告", "操作警告", "操作失败，请稍后重试", "操作已取消");
                this.getList();
                this.open = false;
