...
<script>
import { getUserInfo, getUserList, confirmToPassTheReview, refuseToPassReview, deleteCertificationRecords, cancelUserInfoAuthentication, adminActivation, deleteAccounts } from '@/api';
...
export default {
    ...
    methods: {
        ...
        async cancelForm() {
            ...
            this.handleClose();
            this.getUserPage()
            // this.$confirm('确认拒绝通过审核吗？', '提示', {
            //     confirmButtonText: '确定',
            //     cancelButtonText: '取消',
            //     type: 'warning'
            // }).then(() => {
            //     refuseToPassReview(this.form.id).then(res => {
            //         if (res.data.code == 1) {
            //             this.$message({
